...
class System(models.Model):
    ...
    system_id = models.CharField(max_length=64, unique=True, verbose_name='System ID')
    ...
    group = models.ForeignKey('SystemGroup', on_delete=models.SET_NULL, null=True, blank=True, related_name='systems')
    ...
    def __str__(self):
        return f"{self.hostname} ({self.system_id[:8]}...)"
...
class SystemGroup(models.Model):
    ...
    class Meta:
        verbose_name = 'Grupo de Sistemas'
        verbose_name_plural = 'Grupos de Sistemas'
...
class AgentToken(models.Model):
    ...
    system = models.OneToOneField(System, on_delete=models.CASCADE, related_name='agent_token', null=True, blank=True)
...
class Metric(models.Model):
    ...
    class Meta:
        ...
        ordering = ['-created_at']
        indexes = [
            models.Index(fields=['system', 'metric_type', '-created_at']),
        ]
    
    def __str__(self):
        return f"{self.system.hostname} - {self.metric_type}: {self.value} {self.unit}"
...
class AlertRule(models.Model):
    ...
    system = models.ForeignKey(System, on_delete=models.CASCADE, related_name='alert_rules', null=True, blank=True)
...
class Alert(models.Model):
    ...
    def __str__(self):
        return f"{self.system.hostname} - {self.message}"
...
class Deployment(models.Model):
    ...
    system = models.ForeignKey(System, on_delete=models.CASCADE, related_name='deployments')
    ...
    def __str__(self):
        return f"{self.name} - {self.system.hostname} ({self.get_status_display()})"
...
class CommandHistory(models.Model):
    ...
    system = models.ForeignKey(System, on_delete=models.CASCADE, related_name='command_history')
    ...
    def __str__(self):
        return f"{self.system.hostname} - {self.command[:50]}..."
