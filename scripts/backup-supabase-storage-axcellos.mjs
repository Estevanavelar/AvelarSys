#!/usr/bin/env node
/**
 * Backup de Storage - AxCellOS
 * Bucket: axcellos-uploads
 * URL: https://axcellos.avelarcompany.com.br
 */
...
const BACKUP_DIR = join(ROOT, 'backups', 'storage-axcellos');
...
async function main() {
  ...
  // AxCellOS pode ter sua própria URL ou compartilhar o Storage do banco principal
  const url = process.env.VITE_SUPABASE_URL || 'https://banco.avelarcompany.dev.br';
  const key =
  ...
  console.log('📦 Projeto: AxCellOS (Bucket: axcellos-uploads)');
  console.log('---');

  const bucketName = 'axcellos-uploads';
  ...
  console.log(`\n${'='.repeat(60)}`);
  console.log(`✨ Backup AxCellOS concluído!`);
